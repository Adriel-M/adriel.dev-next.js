...
On my previous [post](./2025-04-17-properly-caching-images-in-your-mdx-based-nextjs-blog), one concern with bundling
...
- Identify whether the path is an absolute path from the project root or a relative path from where the `mdx` file is
- Read the file and hash it. I chose [xxhash64](https://xxhash.com/) as the hashing algorithm since I needed a fast algorithm
  and didn't need a cryptographic secure algorithm.
- Copy over the image into the `options.destinationDir` folder with a naming scheme `<name>.<hash>.<extension>`.
